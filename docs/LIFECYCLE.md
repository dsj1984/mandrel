...
| `epic.watch.end` | [`epic.watch.end.schema.json`](../.agents/schemas/lifecycle/epic.watch.end.schema.json) | Emitted by Watcher when required checks settle. checkOutcomes maps check-name → terminal state (success \| failure \| timed_out \| skipped). AutomergePredicate subscribes to this event (test-only `Watcher` path; the production Phase 8.5 boundary is `epic.automerge.start`). | `prUrl`, `checkOutcomes` |
