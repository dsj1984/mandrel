...
| `epic.complete` | [`epic.complete.schema.json`](../.agents/schemas/lifecycle/epic.complete.schema.json) | Terminal event for a successful Epic run, emitted by Cleaner once the temp archive + branch reap complete. Subscribed by LabelTransitioner, which flips the Epic ticket to agent::done (closing it as completed, idempotently); NotifyDispatcher also fans the completion out to the curated webhook channel. | `epicId`, `prUrl` |
