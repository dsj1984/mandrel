...
| `topFrictionCategories` | `array` | Most frequent categories in the current run |

---

## `orchestration.worktreeIsolation` Config

Config block validated by `config-schema.js`. Controls worktree-per-story
execution. Absent or `enabled: false` restores single-tree behavior.
...
| Bulk label-poll path              | Behavior            | `state-poller.js` chooses between a bulk `GET /issues?labels=agent::*&state=open` read and the per-ticket fallback based on tracked-story count and response well-formedness. Malformed payloads fall back to per-ticket; out-of-scope issues are filtered against the tracked-story set. |
...
## Grouped `agentSettings` Contract

`agentSettings` is reorganised into typed sub-blocks. There are no flat-key
reads anywhere in the resolver or in any consumer. The full key catalogue,
defaults, and required-vs-optional flags live in
