...
| `verification-results`      | `lib/orchestration/code-review.js` (`runCodeReview`)                    | Unified review findings on the Story; critical findings block close. Read by the feedback-loop graduators and the auto-merge integration gate. |
...
| `--only mi\|crap`                                   | CLI flag           | On `quality-preview.js`. Runs one half only — maintainability (`mi`) or CRAP (`crap`); the other half is reported as not run. Absent, both halves run serially (the `pre-push` shape). Close registers each half as its own gate. |
...
| `lib/audit-suite/index.js`                 | SDK barrel | `selectAudits()` plus the write-time checklist helpers. Selection reads `audit-rules.json` (schema `audit-rules.schema.json`). Imported by `run-epilogue.js` and the checklist threading — `runAuditSuite()` retired with the close-time lens pass (Story #5416), and there is no audit-suite CLI. |
...
| Field               | Meaning                                                                              |
| ------------------- | ------------------------------------------------------------------------------------ |
| `gateName`          | Lowercase gate identifier — closed enum in `.agents/schemas/validation-evidence.schema.json`: `typecheck`, `lint`, `test`, `format`, `coverage-capture`, `check-baselines`, `check-baselines-independent`, `check-baselines-coverage`, `quality-preview`, `quality-preview-mi`, `quality-preview-crap` (the real close-validation gates — Story #4697, Story #5172 for the split baselines pair, where `check-baselines` remains the unsplit fail-closed fallback, and Story #5471 for the pre-push quality preview split into its MI half, run in the parallel phase, and its CRAP half, run serially after `coverage-capture`; `quality-preview` is the historical name of the unsplit gate from Story #5378), plus retired `check-maintainability` / `check-crap` kept so historical evidence records still validate. |
