...
- **Digest + artifact path.** When a script produces a large result (a full
  ...
  [`lib/observability/terse-result.js`](../../.agents/scripts/lib/observability/terse-result.js)
...
- **Streamed child output counts too (Story #4736).** A script that pipes a
  ...
  ([`single-story-close/gate-log.js`](../../.agents/scripts/lib/orchestration/single-story-close/gate-log.js)),
