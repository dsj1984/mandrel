...
describe('migrations module exports', () => {
  ...
  // the lintBaseline-command retirement at 2.32.0; Story #5312 appended the
  // planning-limit-knob retirement at 2.57.0; Story #5313 appended the
  // delivery-limit-knob retirement at the same version; Story #5366 appended
  // the auditResultsAutoFile retirement at 2.60.0; Story #5410 appended the
  // CLAUDE.md → AGENTS.md fold at 2.65.0. The registry's
  // declaration order is what fixes the run order within a version — pin it.
  it('ships the real steps in ascending version + declaration order', () => {
    ...
    assert.match(migrations[6].description, /complexityGate/);
    assert.match(migrations[7].description, /freshCriticSampleRate/);
    assert.match(migrations[8].description, /auditResultsAutoFile/);
    assert.match(migrations[9].description, /#5382/);
    assert.match(migrations[10].description, /generatedAt and rollup/);
    assert.match(migrations[11].description, /CLAUDE\.md into AGENTS\.md/);
  });
});
