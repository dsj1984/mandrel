...
/**
 * Build the stubbed `deps` for `run`. The post-install phases (sync,
 * sync-commands, migrate, doctor) run through the spawn boundary, which is
 * stubbed via `spawnFn` to return status 0 — no real child process, and the
 * doctor phase passes. The boundary under test is the changelog surface.
 *
 * @param {{
...
function makeDeps(fs, cap, fetchChangelog) {
  return {
    ...
    // Stub the post-install spawn boundary (the sole post-install path since
    // Story #4182): every phase, including doctor, exits 0.
    spawnFn: () => ({ status: 0, stdout: '', stderr: '' }),
    // Story #4613 — the re-exec resolves the bin *script* from the consumer
    // root; stub it so these changelog tests need no real `mandrel` on disk.
    resolveBinScript: () =>
