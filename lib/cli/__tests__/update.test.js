...
describe('defaultVersionRunner — win32 spawn shape', () => {
  it('passes shell matching the platform to spawnSync (CVE-2024-27980)', () => {
    ...
    assert.deepEqual(calls[0].args, ['view', '@mandrelai/agents', 'version']);
  ...
  it('throws a descriptive error when the probe spawn errors (ENOENT)', () => {
    ...
    assert.throws(
      ...
      /failed to probe newest @mandrelai\/agents version: spawnSync npm ENOENT/,
    );
  });
});
...
describe('defaultNpmUpdate — win32 install spawn shape', () => {
  it('routes the install through runInstallCommand with the win32 shell flag', () => {
    ...
    assert.deepEqual(calls[0].args, ['install', '@mandrelai/agents@1.46.0']);
  ...
  it('throws when the install command exits non-zero', () => {
    ...
    assert.throws(
      ...
      /install command `npm install @mandrelai\/agents@1\.46\.0` exited 1: boom/,
    );
  });
});
...
describe('resolveInstallCmd — default vs override', () => {
  it('defaults to npm install @mandrelai/agents@<target> when no override', () => {
    assert.equal(
  ...
  it('uses the operator override verbatim when supplied', () => {
    assert.equal(
      resolveInstallCmd('1.46.0', 'pnpm add @mandrelai/agents@latest'),
...
describe('runUpdate — --install-cmd flag threading', () => {
  ...
  it('threads the --install-cmd value into npmUpdate', async () => {
    ...
    const cap = makeCapture();
    await runUpdate({
      argv: ['--install-cmd', 'pnpm add @mandrelai/agents@1.44.0'],
      currentVersion: '1.43.0',
      resolveTargetVersion: async () => '1.44.0',
      npmUpdate: async (_target, opts) => {
        seen.push(opts);
    ...
    assert.equal(seen.length, 1);
    assert.equal(seen[0].installCmd, 'pnpm add @mandrelai/agents@1.44.0');
  });
  ...
  it('accepts the --install-cmd=<value> form', async () => {
    ...
    const cap = makeCapture();
    await runUpdate({
      argv: ['--install-cmd=yarn up @mandrelai/agents@1.44.0'],
      currentVersion: '1.43.0',
      resolveTargetVersion: async () => '1.44.0',
      npmUpdate: async (_target, opts) => {
        seen.push(opts);
    ...
    assert.equal(seen[0].installCmd, 'yarn up @mandrelai/agents@1.44.0');
  });
});
...
describe('update default export — install routing', () => {
  ...
  it('runs the overridden install command through the injected runInstall seam', async () => {
    const installs = [];
    await update(['--install-cmd', 'pnpm add @mandrelai/agents@1.44.0'], {
      currentVersion: '1.43.0',
      fs: makeMemoryFs(),
    ...
    assert.equal(installs[0].cmd, 'pnpm add @mandrelai/agents@1.44.0');
  });
  ...
  it('runs the default npm install command when no override is given', async () => {
    ...
    assert.deepEqual(installs, ['npm install @mandrelai/agents@1.44.0']);
  });
});
