...
function makeSeams({ target = '1.44.0', doctorOk = true } = {}) {
  ...
  return {
    ...
    cwd: () => '/fake/consumer',
    // Story #4613 — stub the bin-script resolver so the fake cwd needs no real
    // `mandrel` install on disk (the re-exec resolves <root>/bin/mandrel.js).
    resolveBinScript: () =>
...
describe('runUpdate — --install-cmd flag threading', () => {
  ...
  it('threads no installCmd into npmUpdate by default', async () => {
    ...
    await runUpdate({
      ...
      // root; stub it so the fake cwd needs no real `mandrel` on disk.
      resolveBinScript: () =>
        '/fake/consumer/node_modules/mandrel/bin/mandrel.js',
      write: cap.write,
      writeErr: cap.writeErr,
...
describe('update default export — install routing', () => {
  ...
  it('runs the overridden install command through the injected runInstall seam', async () => {
    ...
    await update(['--install-cmd', 'pnpm add mandrel@1.44.0'], {
      ...
      spawnFn: okSpawnFn,
      // Story #4613 — the re-exec resolves the bin *script* from the consumer
      // root; stub it so the default export needs no real `mandrel` on disk.
      resolveBinScript: () =>
        '/fake/consumer/node_modules/mandrel/bin/mandrel.js',
      write: () => {},
      writeErr: () => {},
