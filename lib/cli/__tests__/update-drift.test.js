...
function makeUpToDateSeams({ drifted = false } = {}) {
  ...
  return {
    ...
    cwd: () => '/fake/consumer',
    // Story #4613 — the re-exec resolves the bin *script*; stub it so the fake
    // cwd needs no real `mandrel` install on disk.
    resolveBinScript: () =>
...
describe('runUpdate — drift-aware no-op (Story #4065)', () => {
  ...
  it('invokes spawnPhase for sync + sync-commands + sync-agents on the re-exec drift-heal path', async () => {
    const spawnCalls = [];
    const cap = makeCapture();
    ...
    // Exactly sync + sync-commands + sync-agents, no migrate, no doctor on
    // the heal path.
    const phaseNames = spawnCalls.map((c) => c.phase);
    assert.deepEqual(phaseNames, ['sync', 'sync-commands', 'sync-agents']);
