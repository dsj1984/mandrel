 ...
 * for Story #4046 A1c (sync-commands step added to STEP_PLAN).
 *
 * After `npm-update` lands the new version, the post-install phases
 * (sync, sync-commands, migrate, doctor) MUST run from the newly-installed
 * binary rather than in the still-running old process. The `spawnPhase` seam
 * is the injectable boundary that makes this fully testable without a real npm
 * install.
 *
 * Coverage contract (Story #4034 + Story #4046 A1c; Story #4182 made
 * `spawnPhase` the sole post-install path — No-Shim):
 *   - Post-install phases run through `spawnPhase` (the re-exec boundary).
 *   - Phases are spawned in the correct order:
 *     sync → sync-commands → migrate → doctor.
...
describe('runUpdate — re-exec path via spawnPhase', () => {
  it('routes post-install phases through spawnPhase, not in-process seams', async () => {
    ...
    // npm-update ran, then all four phases were spawned via spawnPhase
    // (sync → sync-commands → migrate → doctor — Story #4046 A1c).
  ...
  it('passes correct ordered argv to each phase: sync→sync-commands→migrate(--from,--to)→doctor', async () => {
    const cap = makeCapture();
    ...
    // sync, sync-commands, migrate, doctor = 4 phases (Story #4046 A1c)
    ...
    assert.equal(phases[2].phase, 'migrate');
    assert.deepEqual(phases[2].args, ['--from', '1.43.0', '--to', '1.50.0']);

    assert.equal(phases[3].phase, 'doctor');
    assert.deepEqual(phases[3].args, []);
  });
