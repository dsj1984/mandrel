...
describe('runUpdate — STEP_PLAN includes sync-commands (A1c)', () => {
  it('--dry-run output lists sync-commands between runSync and runMigrations', async () => {
    ...
    // All five core steps plus changelog must appear.
    ...
    assert.ok(
      syncCmdIdx < migrateIdx,
      'sync-commands must precede runMigrations in the step plan',
    );
  });

  it('dry-run does not hard-code a fifth step outside STEP_PLAN', async () => {
    const cap = makeCapture();
    await runUpdate({
      argv: ['--dry-run'],
      currentVersion: '1.43.0',
    ...
    const joined = cap.out.join('');
    // The step count in the output should match STEP_PLAN length (6 steps:
    // npm-update, runSync, sync-commands, runMigrations, doctor,
    // surface changelog). Verify the step-list lines match.
    const stepLines = joined.split('\n').filter((l) => /^\s+\d+\./.test(l));
    assert.equal(
      stepLines.length,
      6,
      `expected 6 numbered step lines; got:\n${stepLines.join('\n')}`,
    );
  });
});
...
describe('runUpdate — re-exec path spawns sync-commands before migrate (A1c)', () => {
  it('spawns sync → sync-commands → migrate → doctor in that order', async () => {
    const cap = makeCapture();
    const spawnCalls = [];
