...
import sync, { readVersionMarker, runSync } from '../sync.js';
...
/** Seed a package payload of two files under <pkg>/.agents/, plus the
 * package's own package.json (Story #4530 — the version marker reads this). */
...
describe('runSync — copies .agents/ payload as plain files', () => {
  ...
  it('reports the installed file count on stdout', () => {
    const fs = makeFs(seedPackagePayload());
    const cap = makeCapture();
    runSync({
      argv: [],
    ...
    assert.match(cap.out.join(''), /Installed 2 file\(s\)/);
  });
});
...
// ---------------------------------------------------------------------------
// Story #4530 — version marker: write, exemption, and reader degrade path
// ---------------------------------------------------------------------------

describe('runSync — writes .agents/.mandrel-version', () => {
  it('writes the executing package version, trimmed of surrounding whitespace on read', () => {
    const fs = makeFs(seedPackagePayload());
    const cap = makeCapture();
    runSync({
      argv: [],
    ...
    const markerPath = path.join(PROJECT, '.agents', '.mandrel-version');
    assert.equal(fs.files.get(markerPath), '9.9.9\n');
  });

  it('does not write the marker on the missing-package early exit', () => {
    const fs = makeFs();
    const cap = makeCapture();
    runSync({
      argv: [],
    ...
    assert.equal(
      fs.files.has(path.join(PROJECT, '.agents', '.mandrel-version')),
      false,
    );
  });

  it('does not write the marker on --dry-run', () => {
    const fs = makeFs(seedPackagePayload());
    const cap = makeCapture();
    runSync({
      argv: ['--dry-run'],
...
describe('readVersionMarker', () => {
  it('resolves the version when the marker is present', () => {
    const fs = makeFs({
      [path.join(PROJECT, '.agents', '.mandrel-version')]: '9.9.9\n',
    });
    assert.equal(readVersionMarker(PROJECT, fs), '9.9.9');
  });

  it('returns null when the marker is absent (pre-marker install) rather than throwing', () => {
    const fs = makeFs({});
    assert.equal(readVersionMarker(PROJECT, fs), null);
  });

  it('returns null for an empty marker file instead of an empty string', () => {
    const fs = makeFs({
      [path.join(PROJECT, '.agents', '.mandrel-version')]: '',
    });
    assert.equal(readVersionMarker(PROJECT, fs), null);
  });
});
