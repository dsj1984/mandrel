...
/**
 * Strip the framework system-prompt import block from a `CLAUDE.md`. When the
 * file is left with no meaningful content beyond a bare title, it is removed
 * (the install authored it). A file that still carries operator content is
 * preserved with only the framework block excised.
 *
...
function revertGitignore(projectRoot, fsImpl) {
  ...
  const original = fsImpl.readFileSync(target, 'utf8');
  let next = original;
  for (const def of Object.values(GITIGNORE_BLOCKS)) {
  ...
  return {
    kind: 'reverted',
    target: rel,
    detail: 'removed framework ignore entries',
  };
}
...
/**
 * Reverse the `.husky/pre-commit` quality hook. When the hook carries only
 * the framework's quality-preview line (install authored it), the file is
 * removed; when the operator merged the line into a custom hook, only the
 * framework line is stripped.
 *
 * @param {string} projectRoot
 * @param {typeof fs} fsImpl
 * @returns {ReversalOutcome}
...
function revertPreCommitHook(projectRoot, fsImpl) {
  ...
  // Treat a body of only blank lines / the install's own comment header as
  // framework-authored, so the install-created hook is removed cleanly.
