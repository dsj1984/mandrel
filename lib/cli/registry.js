...
/**
 * Fatal doctor check (Story #4525 / #4530): does the consumer's declared
 * `mandrel` dependency pin (`package.json`) agree with the version actually
 * resolvable in `node_modules`?
 *
 ...
 * that has fallen behind what `node_modules` actually resolves to — e.g. an
 * out-of-band symlink, or `npm install mandrel@latest --no-save` — which
 * `mandrel doctor` graded as fully green before this check existed, because
 * the only version `defaultCurrentVersion` ever read WAS the inflated
 * `node_modules` one.
 *
 * **Fatal, unlike `version-current`.** `version-current` is a deliberately
 * non-fatal, cache-only advisory ("is a newer version published?") and this
 * Story does not change that contract — see its doc comment below. A pin
 * that disagrees with what is actually installed is a different, stronger
 * claim: the declared dependency does not describe the code that is
 * running, which is exactly the kind of skew a `failed` check exists to
 * surface rather than quietly heal.
 *
 ...
 * range) — via {@link resolveConsumerPinVersion}'s own `null` contract. This
...
export function runPinCurrent({ cwd, fsImpl = fs, resolvePackageRoot } = {}) {
  ...
  const pinned = resolveConsumerPinVersion(projectRoot, fsImpl);
  if (!pinned) {
    return {
      ok: true,
  ...
  if (compareSemver(pinned, installed) !== 0) {
    return {
      ok: false,
      detail: `package.json pins v${pinned} but v${installed} is installed`,
      remedy:
        'Run `mandrel update` to reconcile the package.json pin with the installed version.',
    };
  }

  return { ok: true, detail: `pin v${pinned} matches the installed version` };
}
