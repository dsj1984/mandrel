 ...
 * sources to the generated plugin command tree
...
function runCommandsInSync({ projectRoot, readDir } = {}) {
  ...
  const destDir = path.join(root, '.claude', 'plugins', 'mandrel', 'commands');
  ...
  if (missing.length === 0 && extra.length === 0) {
    return { ok: true, detail: `${sources.length} plugin commands up to date` };
  }
  ...
  return {
    ...
    detail: parts.join(', '),
    remedy:
      'Run `npm run sync:commands` to regenerate the mandrel plugin command tree.',
  };
}
