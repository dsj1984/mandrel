...
import { write as writeEnvelope, writeFile as writeEnvelopeFile } from '../../.agents/scripts/lib/baselines/writer.js';
...
async function resolveScope({
  ...
  const predicate = KIND_FILE_PREDICATES[kind];
  const rawFiles = await gitDiff({ baseRef, headRef, cwd });
  const files = [];
  for (const raw of rawFiles ?? []) {
    if (typeof raw !== 'string' || raw.length === 0) continue;
    const canonical = canonicalizeBaselinePath(raw);
    if (predicate(canonical)) files.push(canonical);
  }
