...
- Honour the Story-only hierarchy: every ticket is a **Story**. Stories carry the implementation scope inline (`## Spec` + acceptance/verify); there is no Feature tier, and no Epic/Task ticket tier exists. Thematic grouping is prose in the Spec, never a parent ticket.
