---
description: >-
  Phase 7 of sprint planning — generate the Tech Spec and Acceptance Spec for
  an Epic, persist them as linked GitHub issues, and flip the Epic to
...
Acceptance Spec — persists them as `context::tech-spec` and
`context::acceptance-spec` issues under the Epic, and flips the Epic to
`agent::review-spec` (parking) so a human reviewer can read the artifacts on
GitHub before decomposition. The PRD artifact class was retired (Story #4314);
its one novel section, **User Stories**, now lives inline in the Epic body.

> **Single prose home.** The canonical, full-detail spec-phase contract
> (idempotent context tickets, the two-ticket rationale, the
...
The Tech Spec and Acceptance Spec are authored **directly by you, the host
LLM**. `epic-plan-spec.js` is a deterministic wrapper that (a) emits the
authoring context you need and (b) persists the artifacts and transitions the
...
- **Do not** create or modify tickets outside the
...
## Step 2.6 — Author the Acceptance Spec

Using `systemPrompts.acceptanceSpec`, the Epic body, and the Tech Spec, write
the Acceptance Spec to `temp/epic-[Epic_ID]/acceptance-spec.md`. It captures the
...
```bash
# Normal flow (two context tickets)
...
# Re-plan (--force overwrites the context tickets in place)
node .agents/scripts/epic-plan-spec.js --epic [Epic_ID] \
  --techspec temp/epic-[Epic_ID]/techspec.md \
  --risk-verdict temp/epic-[Epic_ID]/risk-verdict.json \
...
- Creates `[Tech Spec]` and `[Acceptance Spec]` child issues
...
- Upserts the `epic-plan-state` structured comment with the current phase,
  Tech Spec / Acceptance Spec IDs, the `riskVerdict` field, and
  timestamps.
...
- **High risk or `--force-review` — STOP.** Surface the Tech Spec and
  Acceptance Spec URLs to the operator:

  > "Spec phase complete for Epic #[ID]. Review Tech Spec (#YY) and
  > Acceptance Spec (#ZZ) on GitHub. When you're ready, re-run
  > `/plan [Epic_ID]` — the wrapper will pick up where it left off and run
  > the decompose phase."
...
- If the persist call fails after creating the Tech Spec but before the
  Acceptance Spec, re-run with `--force` (the script reuses the existing Tech
  Spec when appropriate).
- If the Epic does not flip to `agent::review-spec` after the script claims
  ...
  persist step; it's idempotent against the existing Tech Spec.
