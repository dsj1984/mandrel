...
/**
 * Story #2193 / Task #2201 — AC-6: fail closed when a configured floor
 * axis is not present in the rollup. Pre-#2193 the missing axis was
 * silently skipped inside `compareToFloor` (which lets a typo like
 * `{ maintainability: 70 }` against a `{ min, p50, p95 }` rollup pass as
 * "no breach"); now `applyFloors` invokes this guard *before* the
 * silent-skip comparator so the misconfiguration surfaces immediately.
 *
 * Throws an `Error` whose `.code` is `'EXIT_CONFIG'`. The CLI shell in
 * `main()` maps a thrown error from `runCheckBaselines` to exit code 3
 * (config-resolution failure) — the same exit-code class as a malformed
 * `.agentrc.json`, which is the right bucket: a floor that names an axis
 * the rollup does not expose is a `.agentrc.json` typo, not a baseline
 * data failure.
 *
 * The thrown message follows the contract: it names the configured floor
 * axis and the available rollup keys so the operator can fix the
 * config in one read (e.g. `did you mean 'min'?`).
 *
 * Pure; exported for tests.
 *
 * @param {string} kind
 * @param {string} component  the rollup component name (`*` for whole-repo)
 * @param {Record<string, number>} aggregate  the rollup row for `component`
 * @param {Record<string, number>} floor      the configured floor for `component`
 * @returns {void}
 */
 ...
 * misnamed a floor axis (e.g. `maintainability` against a rollup that
 * exposes `min` / `p50` / `p95`). Substring containment is enough — the
 * canonical pre-#2193 typo is `maintainability` mapping to `min` (because
 * the kind name itself is `maintainability`), so we fall back to the
 * first available key when no character-overlap match scores high enough.
 * Pure.
 */
...
export function applyFloors(kind, rollup, floors) {
  ...
  for (const component of components) {
    ...
    // Story #2193 / AC-6: validate axis-name correspondence *before* the
    // silent-skip comparator so an `.agentrc.json` typo
    // (`maintainability: 70` against a `{ min, p50, p95 }` rollup) trips
    // the gate with an actionable error instead of passing as "no breach".
