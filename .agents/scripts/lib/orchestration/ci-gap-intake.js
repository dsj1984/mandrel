/**
 * ci-gap-intake.js — `rules/ci-remediation.md` Option 2: file an intake issue
 * in the repo that owns the fault; `/mandrel-plan <id>` graduates it later
 * (never called from here — this runs unattended). A repeat signature lands
 * on the existing ticket: the occurrence count is the evidence. Ports are
 * injected.
 */
...
/** Contract with `/mandrel-plan` Phase 0; bump only alongside its reader. */
export const CI_GAP_INTAKE_MARKER = '<!-- ci-gap-intake: v1 -->';

/** Each becomes a `friction::<verdict>` label counted by Phase 0. */
...
/** Option 1: a defect in the diff is fixed on the branch, never filed. */
...
/**
 * Strip run-specific noise so repeat occurrences fingerprint identically.
 *
 * @param {string} line
 * @returns {string}
 */
...
/**
 * `area` folds in the verdict: the same signature under two verdicts is two
 * different owners' work.
 *
 * @param {object} opts
...
/**
 * Makes a mis-route visible: names the destination or the missing key.
 *
...
/**
 * Grafts a fresh table when a hand-edited body lost it.
 *
 * @param {string} body
 * @param {object} occurrence
 * @returns {string}
...
function appendOccurrence(body, occurrence) {
  ...
  // Insert at the table's end, not the body's, so the footer stays intact.
...
/**
 * A refused cross-repo write degrades to a local filing naming the owning
 * repo; `routing` is mutated so envelope and body agree.
 *
 * @param {object} opts
...
/**
 * Port failures land in `errors[]`; only an invalid verdict or bucket throws,
...
export async function fileCiGapIntake({
  ...
  // Unroutable → file locally and say so; never look routed.
  const routing = {
    bucket,
  ...
  try {
    ...
    // A degraded lookup stops rather than guessing `new` and duplicating.
  ...
  // Only `new` or a closed-ticket regression earns a fresh ticket.
  const isRecurrence =
