 ...
 *     `type::story`, `audit::<lens>` and — as a bare string literal —
 *     `risk::high`.
 *
...
const AUDIT_STORY_AXIS_LABELS = Object.freeze([
  ...
  // Created, never generated — the one deliberate asymmetry in this module.
  // The filer emits no `agent::` state (see `STATIC_LABELS` in
  // build-story-body.js): a freshly-filed audit Story is not delivery-ready,
  // and `/mandrel-plan` stamps this label as its terminal flip after
  // enrichment. The bootstrap still creates it so that flip has a label to
  // apply on a repo where only the audit bootstrap has been run. The drift
  // this module guards runs the other way — a generator inventing names the
  // creator never made — so a creator-only entry is safe (Story #5229).
  {
