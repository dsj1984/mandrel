...
//      blocklist is seeded with `agents-bootstrap-github` and takes
...
export const RETIRED_COMMANDS = new Set(['agents-bootstrap-github']);
...
export function checkFile(absPath, repoRoot) {
  ...
  //    the mandrel plugin command tree but are still legitimate named
  //    workflows that parent workflows invoke by prose reference).
  for (const { token, line } of slashTokens) {
