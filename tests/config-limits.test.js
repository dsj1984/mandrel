...
// envelope. `maxTokenBudget` was retired; `getLimits` never exposes it.
// ---------------------------------------------------------------------------
...
describe('maxTokenBudget absent from LIMITS_DEFAULTS (Story #3875)', () => {
  ...
  it('getLimits never returns maxTokenBudget', () => {
    assert.equal('maxTokenBudget' in getLimits({}), false);
    assert.equal('maxTokenBudget' in getLimits(undefined), false);
    assert.equal(
      'maxTokenBudget' in getLimits({ delivery: { maxTokenBudget: 12345 } }),
      false,
    );
  });
