// tests/contract/security-report-contract.test.js
//
// Contract tier (Epic #3597, Story #3613): the `audit-security` report
...
} from '../../.claude/workflows/audit-security.workflow.js';
import {
  ...
  REQUIRED_SECTIONS,
  SEVERITY_LEVELS,
} from '../../scripts/lib/dynamic-workflow/security-report-contract.js';
...
const LENS = readFileSync(
  path.join(REPO_ROOT, '.agents', 'workflows', 'audit-security.md'),
  'utf8',
);
...
test('lens markdown writes the canonical artifact basename', () => {
  assert.ok(
    ...
    'lens Step 3 must target {{auditOutputDir}}/audit-security-results.md',
  );
});
...
test('lens markdown references the severity taxonomy', () => {
  for (const level of SEVERITY_LEVELS) {
    assert.ok(SOURCE.includes(level), `lens missing severity level: ${level}`);
  }
});
...
test('assertReportContract: a report missing a section is non-conformant', () => {
  const report = [
    ...
    '## Executive Summary',
    '## Detailed Findings',
    // Defensive Recommendations deliberately omitted
  ].join('\n');
  const result = assertReportContract(report);
  assert.equal(result.conformant, false);
  assert.deepEqual(result.missingSections, ['Defensive Recommendations']);
});
