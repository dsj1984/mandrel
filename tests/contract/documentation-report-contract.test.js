// tests/contract/documentation-report-contract.test.js
//
// Contract tier (Story #4024): the `audit-documentation` report contract is
// the boundary both execution paths cross. These tests assert that:
...
const LENS = readFileSync(
  path.join(REPO_ROOT, '.agents', 'workflows', 'audit-documentation.md'),
  'utf8',
);
...
test('lens markdown writes the canonical artifact basename', () => {
  assert.ok(
    ...
    'lens Step 3 must target {{auditOutputDir}}/audit-documentation-results.md',
  );
});
...
test('lens markdown references the impact taxonomy', () => {
  for (const level of IMPACT_LEVELS) {
    assert.ok(LENS.includes(level), `lens missing impact level: ${level}`);
  }
});
...
test('orchestrated synthesis prompt folds in deterministic-gate findings', () => {
  const withFindings = buildSynthesisPrompt(
    [],
    'temp/audits',
    '### Broken link\n- **Category:** Link Integrity',
  );
  assert.ok(withFindings.includes('Link Integrity'));
  const without = buildSynthesisPrompt([], 'temp/audits');
  assert.ok(without.includes('(none reported by the calling session)'));
});

// --- scope parity: both paths honour the {{changedFiles}} contract ---------

test('buildScopeClause: unsubstituted token → full target-set audit', () => {
  assert.match(buildScopeClause('{{changedFiles}}'), /full config-driven/i);
  assert.match(buildScopeClause(''), /full config-driven/i);
  assert.match(buildScopeClause(undefined), /full config-driven/i);
});

test('buildScopeClause: a real file list → scoped analysis', () => {
  const clause = buildScopeClause('docs/a.md\nREADME.md');
  assert.match(clause, /Restrict analysis/i);
  assert.ok(clause.includes('docs/a.md'));
  assert.ok(clause.includes('README.md'));
});
