/**
 * tests/lib/retro-substrate.test.js — Story #4417 ("Retro reads the truth").
 *
 * Pins the four behavioural cutovers that make the Epic retro read from ONE
 * truthful substrate:
 *
 *  1. `retro-run.js` resolves framework/consumer repos from
 *     `resolveConfig().github` and threads them through `runRetro` to
 *     `gatherRetroSignals`. The consumer repo has NO silent fallback: a
 *     missing github config disables the consumer pane loudly instead of
 *     routing consumer-tagged friction at `dsj1984/mandrel`.
 *  2. `counts.friction` derives from the per-Story `signals.ndjson` scan
 *     (the same scan that feeds the routed proposals), so a categorized
 *     signal on disk with NO `story-perf-summary` comment still forces the
 *     full retro shape and `cleanSprint: false`.
 *  3. `composeRetroBody` never returns the compact shape when
 *     `routedProposals` holds a non-empty actionable bucket — regardless of
 *     `forceFull` or the comment-derived counts (auto-merge input).
 *  4. Unresolved `agent::blocked` Stories read from the epic-run-state
 *     snapshot reach `composeRoutedProposals` as `unresolvedBlockedEvents`
 *     with a non-empty category (fallback `agent-blocked`) and force an
 *     actionable proposal.
 *  5. The unified counts scan folds in the per-Epic `signals.ndjson` stream
 *     (`appendEpicSignal` writers such as lifecycle-emit) alongside the
 *     per-Story streams.
 */
...
test('resolveRetroRepos: consumer repo comes from github.owner/repo; framework from github.frameworkRepo', () => {
  assert.deepEqual(
    resolveRetroRepos({ github: { owner: 'acme', repo: 'widget' } }),
    { frameworkRepo: '', consumerRepo: 'acme/widget' },
  );
  assert.deepEqual(
    resolveRetroRepos({
      github: {
    ...
    { frameworkRepo: 'dsj1984/mandrel', consumerRepo: 'acme/widget' },
  );
});

test('resolveRetroRepos: absent github config yields empty slugs (no dsj1984/mandrel fallback)', () => {
  assert.deepEqual(resolveRetroRepos(undefined), {
    frameworkRepo: '',
    consumerRepo: '',
  });
  assert.deepEqual(resolveRetroRepos({}), {
    frameworkRepo: '',
    consumerRepo: '',
  });
});

test('runRetroCli: threads resolved repos from injected config into runRetro', async () => {
  let captured = null;
  const out = await runRetroCli({
    epicId: 42,
    injectedConfig: { github: { owner: 'acme', repo: 'widget' } },
    injectedProvider: {},
    busFactory: () => ({ emit() {} }),
    ledgerFactory: () => ({ register() {}, ledgerPath: null }),
  ...
  assert.equal(captured.consumerRepo, 'acme/widget');
  assert.equal(captured.frameworkRepo, '');
});
